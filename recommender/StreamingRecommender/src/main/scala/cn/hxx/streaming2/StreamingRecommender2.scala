...
object ConnHelper extends Serializable {
    ...
    lazy val mongoClient = MongoClient(MongoClientURI("mongodb://myCent:27017/recommender2"))
}
...
object StreamingRecommender2 {
    ...
    def main(args: Array[String]): Unit = {
        val config = Map(
            ...
            "kafka.topic" -> "recommender2" //kafka.topic
        )

        val sparkConf = new SparkConf().setMaster(config("spark.cores")).setAppName("StreamingRecommender2")
        ...
        val ratingStream = kafkaDStream.map {
            msg =>
                ...
                (attr(0).toInt, attr(1).toInt, attr(2).toDouble, attr(3).trim)
        }
