...
object ContentRecommender2 {
    ...
    def main(args: Array[String]): Unit = {
        ...
        }.filter(!_._3.isEmpty).toDF("uid", "mid", "content")   //过滤空值，并指定列名
...
//        rescaledData.show(5,truncate = false)
        ...
        val userFeatures = rescaledData.map(
            // 这里目标格式是(uid, features), 即(Int, Array[Double]) (参照OfflineRecommender)
...
//***************************************************************************************************************
        ...
        val movieTagsDF = movieRaw.map{
            case (mid, name, genres, describe) =>
                val dbc_describe = AsciiUtil.sbc2dbcCase(describe) //全角转半角
                val participle_describe = participleFunc(dbc_describe) //对电影简介分词
                        .split(" ")
                        .filter(
                            x => !broadcast_StopWords.value.contains(x) //过滤停用词
                                    && key_list.contains(x)) //判断是否在word2vec词典内
                val similar_list = mutable.HashSet[String]() //用HashSet去重
                for (item <- participle_describe) { //遍历刚才得到的分词
                    val like = word2vec.findSynonyms(item, 2) //找相近的两个词
                    for ((item, cos) <- like) {
                        similar_list += item
                    }
                }
                similar_list ++= participle_describe     //保留原始分词
                (mid, similar_list.mkString(" "))
        }.toDF("mid", "describe").cache()

        val tokenizer1 = new Tokenizer().setInputCol("describe").setOutputCol("words")
        // 用分词器对原始数据做转换，生成新的一列words
        // [Drama Horror Thriller] => [drama, horror, thriller]
        ...
        // 相当于hash散列，50是设定的hash表长度，表示这三个类别散列到在26 27 36的位置上，值为1
        // 其类型是SparseVector(稀疏矩阵), 并且用三元组表示[hash表长, 数据位置, 数据]
        val hashingTF1 = new HashingTF().setInputCol("words").setOutputCol("rawFeatures").setNumFeatures(100)
        val featurizedData1 = hashingTF1.transform(wordsData1)

        //        featurizedData1.show(truncate = false)
...
//        rescaledData1.show(truncate = false)
        ...
        // (后面全是复制Offline里的)对电影和用户的vector两两计算相似度
